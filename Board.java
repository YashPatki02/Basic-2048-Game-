...
//
// Author:             Yash Patki, ypatki@ucsd.edu
...
// Persons:          None.
//
// Online sources:   ZyBooks, lecture recordings, lecture slides.
//////////////////////////// 80 columns wide //////////////////////////////////
...
// > java --module-path ./lib --add-modules javafx.controls,javafx.fxml Gui2048

import java.util.*;
import java.io.*;

/**
 * Create a square board and implement methods to be repilcate
 * the game 2048 - count score, check canMove method, and move
 * method
 *
 * Bugs: None known.
 *
...
public class Board {
  ...
  public void printBoard() {
    ...
    //Print each element formatted with 5 width
  ...
  public void printBoard(String direction) {
    ...
    int[][] copyGrid = getGrid();
    
    //Move in the direction passed in and print output
    move(direction);
    printBoard();
  ...
  public boolean canMove(String direction){
    ...
    /* Switch block that DrJava did not let me compile with error - 
     * 
     * Error: Cannot switch on a value of type java.lang.String. 
  ...
  private boolean canMoveUp() {
    ...
    for (int row = 0; row < this.GRID_SIZE-1; row++) {
      for (int col = 0; col < this.GRID_SIZE; col++) {
        ...
        //If the element on top is 0, it can move up
        if (grid[row][col] == 0 && grid[row+1][col] != 0) {
          return true;
        }
        //If two consecutive elements match
  ...
  private boolean canMoveDown() {
    ...
    for (int row = this.GRID_SIZE-1; row > 0; row--) {
      for (int col = 0; col < this.GRID_SIZE; col++) {
        ...
        //If the element on bottom is 0, it can move down
        if (grid[row][col] == 0 && grid[row-1][col] != 0) {
          return true;
        }
        //If two consecutive elements match
  ...
  private boolean canMoveRight() {
    ...
    for (int row = 0; row < this.GRID_SIZE; row++) {
      for (int col = this.GRID_SIZE-1; col > 1; col--) {

        //If the element on right is 0, it can move right
  ...
  private boolean canMoveLeft() {
    ...
    for (int row = 0; row < this.GRID_SIZE; row++) {
      for (int col = 0; col < this.GRID_SIZE-1; col++) {
        
        //If the element on left is 0, it can move left
        if (grid[row][col] == 0 && grid[row][col+1] != 0) {
          return true;
        }
        //If two consecutive elements match
  ...
  private void moveUp() {
    if (canMove(UP) == true) {
      ...
      //Swap non-zero elements to the front with empty spaces
      ...
      for (int col = 0; col < this.GRID_SIZE; col++) {
        ...
        for (int row = 0; row < this.GRID_SIZE; row++) {
          ...
          if (grid[row][col] > 0) {
            int temp = grid[front][col];
            grid[front][col] = grid[row][col];
            grid[row][col] = temp;
      ...
      //If consecutive elements match, put their sum in the first element
      //and 0 for the second element, add score
      for (int col = 0; col < this.GRID_SIZE; col++) {
        for (int row = 0; row < this.GRID_SIZE-1; row++) {
      ...
      for (int col = 0; col < this.GRID_SIZE; col++) {
        ...
        for (int row = 0; row < this.GRID_SIZE; row++) {
          ...
          if (grid[row][col] > 0) {
            int temp = grid[secondFront][col];
            grid[secondFront][col] = grid[row][col];
            grid[row][col] = temp;
  ...
  private void moveDown() {
    if (canMove(DOWN) == true) {
      ...
      for (int col = 0; col < this.GRID_SIZE; col++) {
        int secondFront = this.GRID_SIZE - 1;
        for (int row = this.GRID_SIZE-1; row > -1; row--) {
    ...
    if (canMove(LEFT) == true) {
      ...
      for (int row = 0; row < this.GRID_SIZE; row++) {
        ...
        for (int col = 0; col < this.GRID_SIZE; col++) {
          ...
          if (grid[row][col] > 0) {
            int temp = grid[row][front];
            grid[row][front] = grid[row][col];
            grid[row][col] = temp;
      ...
      for (int row = 0; row < this.GRID_SIZE; row++) {
        ...
        for (int col = 0; col < this.GRID_SIZE; col++) {
          ...
          if (grid[row][col] > 0) {
            int temp = grid[row][secondFront];
            grid[row][secondFront] = grid[row][col];
            grid[row][col] = temp;
  ...
  private void moveRight() {
    if (canMove(RIGHT) == true) {
      ...
      //and 0 for the second element, add score
      for (int row = 0; row < this.GRID_SIZE; row++) {
        for (int col = this.GRID_SIZE-1; col > 0; col--) {
